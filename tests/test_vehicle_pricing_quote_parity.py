...
def test_quote_lead_auth_and_user_linkage_parity(client: TestClient) -> None:
    ...
    initial_email_count = len(client.app.state.sent_emails)
    ...
    sent_emails = client.app.state.sent_emails[initial_email_count:]
    quote_email = next(
        email
        for email in sent_emails
        if email["template_key"] == "quote_lead_notification"
    )
    assert "Customer phone: N/A" in quote_email["text_body"]
    assert "Estimated price: 154,000 VND" in quote_email["text_body"]


def test_quote_lead_includes_customer_phone_in_sales_email(client: TestClient) -> None:
    seed = client.app.state.seed_data
    user_token = _issue_access_token(
        user_id=int(seed["customer_id"]),
        email=seed["customer_email"],
    ...
    assert "Customer phone: 0901234567" in quote_email["text_body"]
    assert "Estimated price: 154,000 VND" in quote_email["text_body"]
